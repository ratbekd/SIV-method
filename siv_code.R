...
k <- signc[which.max(ch),1]# the assumed sign for cor(x,u)
if(k!=0){### this loop works if there is endogeneity so that k is not zero
  ...
  b2t=0
  N<-nrow(mydata)
  reps=2### need to change for larger bootsrap
  S=round(N*.999)
  l=1
